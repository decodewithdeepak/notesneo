...
export function filterNotes(notes: Note[], filters: SearchFilters): Note[] {
  ...
  if (filters.subject && filters.subject.trim()) {
    filtered = filtered.filter(note =>
      note.subject.toLowerCase() === filters.subject?.toLowerCase()
    );
  }
...
export function getSubjectsByBranchAndSemester(notes: Note[], branch?: string, semester?: number): string[] {
  let filtered = notes;
  if (branch && branch !== 'All') {
